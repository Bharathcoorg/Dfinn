...
use crate::pallet::{Call, Config, Pallet as FINNMigration, Pallet, *};

use super::*;

// use crate::pallet::Call as FINNMigration;
// use crate::mock::{
//     // FINNMigration,
//     FINN
// };
pub const FINN: u128 = 1000_000_000_000;
benchmarks! {
	...
	mint {
		...
		let amount: T::Balance = 100u128.saturating_mul(FINN).saturated_into();
		...
		eth_hash.as_mut().copy_from_slice(&random_slice);
			...
			assert_ok!(FINNMigration::<T>::set_migration_operational_status(RawOrigin::Root.into(),true));
			// Register relayers
			assert_ok!(FINNMigration::<T>::set_relayer_status(RawOrigin::Root.into(),relayer1.clone(),true));
			assert_ok!(FINNMigration::<T>::set_relayer_status(RawOrigin::Root.into(),relayer2.clone(),true));
			assert_ok!(FINNMigration::<T>::set_relayer_status(RawOrigin::Root.into(),relayer3.clone(),true));

			assert_ok!(FINNMigration::<T>::mint(RawOrigin::Signed(relayer1).into(), beneficiary.clone(),amount,eth_hash));
			assert_ok!(FINNMigration::<T>::mint(RawOrigin::Signed(relayer2).into(), beneficiary.clone(),amount,eth_hash));
	...
	unlock {
		...
		eth_hash.as_mut().copy_from_slice(&random_slice);
		  ...
		  assert_ok!(FINNMigration::<T>::mint(RawOrigin::Signed(relayer1).into(), beneficiary.clone(),amount,eth_hash));
			assert_ok!(FINNMigration::<T>::mint(RawOrigin::Signed(relayer2).into(), beneficiary.clone(),amount,eth_hash));
			assert_ok!(FINNMigration::<T>::mint(RawOrigin::Signed(relayer3).into(), beneficiary.clone(),amount,eth_hash));
