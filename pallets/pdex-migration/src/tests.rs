...
use crate::mock::{new_test_ext, Origin, FINNMigration, Test, FINN};
...
pub fn check_genesis_config() {
	new_test_ext().execute_with(|| {
		assert_eq!(FINNMigration::operational(), false);
		assert_eq!(FINNMigration::mintable_tokens(), 3_172_895 * FINN);
	});
}
...
pub fn set_migration_operational_status_works() {
	new_test_ext().execute_with(|| {
		let non_sudo = 2u64;
		assert_ok!(FINNMigration::set_migration_operational_status(Origin::root(), true));
		assert_noop!(
			FINNMigration::set_migration_operational_status(Origin::signed(non_sudo), false),
			BadOrigin,
		);
		assert_eq!(FINNMigration::operational(), true);
		assert_ok!(FINNMigration::set_migration_operational_status(Origin::root(), false));
		assert_eq!(FINNMigration::operational(), false);
	});
}
...
pub fn set_relayer_status_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(FINNMigration::set_relayer_status(Origin::root(), relayer, true));
		assert_eq!(Relayers::<Test>::get(&relayer), true);
		assert_ok!(FINNMigration::set_relayer_status(Origin::root(), relayer, false));
...
pub fn unlock_tokens_works() {
	new_test_ext().execute_with(|| {
		...
		let valid_amount = 100 * FINN;
		let eth_hash = H256::random();
		assert_ok!(FINNMigration::set_migration_operational_status(Origin::root(), true));
		// Register relayers
		assert_ok!(FINNMigration::set_relayer_status(Origin::root(), relayer1, true));
		assert_ok!(FINNMigration::set_relayer_status(Origin::root(), relayer2, true));
		assert_ok!(FINNMigration::set_relayer_status(Origin::root(), relayer3, true));

		assert_ok!(FINNMigration::mint(
		...
		assert_noop!(
			FINNMigration::unlock(Origin::signed(unknown_beneficiary)),
			Error::<Test>::UnknownBeneficiary
		);
		assert_noop!(
			FINNMigration::unlock(Origin::signed(beneficiary)),
...
pub fn remove_minted_tokens_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(MintableTokens::<Test>::get(), 3_172_895 * FINN - valid_amount);
		assert_eq!(pallet_balances::Pallet::<Test>::total_issuance(), 100 * FINN);

		// Remove the beneficiary's claim, minted tokens and increase the mintable tokens
		assert_ok!(FINNMigration::remove_minted_tokens(Origin::root(), beneficiary));
		...
		assert_eq!(MintableTokens::<Test>::get(), 3_172_895 * FINN);
		// Make sure the total supply is also decreased
		assert_eq!(pallet_balances::Pallet::<Test>::total_issuance(), 0 * FINN);
	})
}
...
pub fn mint_works() {
	new_test_ext().execute_with(|| {
		...
		let non_relayer = 3u64;
		let beneficiary = 4u64;
		let invalid_amount = (3_172_895 + 1) * FINN;
		let valid_amount = 100 * FINN;
		let eth_hash = H256::random();
		...
		assert_noop!(
			FINNMigration::mint(Origin::signed(relayer), beneficiary, valid_amount, eth_hash),
			Error::<Test>::NotOperational,
		);
		assert_ok!(FINNMigration::set_migration_operational_status(Origin::root(), true));
		// Check if only registered relayers can call the mint function
		assert_noop!(
			FINNMigration::mint(Origin::signed(non_relayer), beneficiary, valid_amount, eth_hash),
			Error::<Test>::UnknownRelayer,
		);
		assert_ok!(FINNMigration::set_relayer_status(Origin::root(), relayer, true));
		// Ensure mint function cannot mint more than the amount available for migration
		assert_noop!(
			FINNMigration::mint(Origin::signed(relayer), beneficiary, invalid_amount, eth_hash),
		...
		assert_noop!(
			pallet_balances::Pallet::<Test>::transfer(
				...
				100,
				valid_amount - 1 * FINN
			), // minus 1 FINN is because of existential deposit
			pallet_balances::Error::<Test>::LiquidityRestrictions
		);
		// Unlock tokens should not work before lock period ends
		assert_eq!(FINNMigration::unlock(Origin::signed(beneficiary)).is_err(), true);
		...
		// Unlock tokens
		assert_ok!(FINNMigration::unlock(Origin::signed(beneficiary)));
		...
		// Check balances
		assert_eq!(pallet_balances::Pallet::<Test>::free_balance(100), 99 * FINN);
		assert_eq!(pallet_balances::Pallet::<Test>::free_balance(beneficiary), 1 * FINN);
	});
}
